...
begin
    try
        ...
        cbModulo.DataSource<TModulo>(Modulo.ObterTodosModulos);
        cbModulo.DataBind;
        cbTipo.DataBind;
        Alterou := false;
        MudarTipoModulo;
...
begin
    try
        ...
        cklModuloRotina.DataSource<TModuloRotina>(cbModulo.GetObject<TModulo>.RotinasModulo);
        ...
        begin
            ...
            cklAcessoPermissoes.DataSource<TPermissaoRotina>(moduloRotina.PermissoesRotina);
...
begin
  if Alterou = true then
    if Tlib.MensagemPergunta('As manutenções de acesso para este módulo e tipo de rotina não foram salvas, Deseja realmente mudar sem salvar?', 'Atenção') = mrNo then
    begin
