...
uses
    ...
    REdit, Vcl.ExtCtrls, RSearch, RComboBox,
  ...
  Dal.FabricaSQL, Vcl.CheckLst, RCheckListBox, UBase;
...
uses
  ...
  UI.Acesso.Cadastro, Biz.Rotina, Lib.Persistable;
...
procedure TfrmTeste.BitBtn11Click(Sender: TObject);
begin
  inherited;
  ShowMessage(tlib.Desencriptar(REdit3.Value));
end;
...
begin
    ...
    _typ :=  _ctx.GetType(rotina.ClassType);


     _pro :=  _typ.GetProperties[3];
     ShowMessage(_pro.Name)  ;

    begin
        for _ca in _pro.GetAttributes do
        begin

            if (_ca is TMaxLenght) then
                ShowMessage('Joao Gabriel')  ;


            if (_ca is TPersistable) then
                ShowMessage('Manutencao');
...
//
  ...
  RCheckListBox1.DataSource<TFilial>(listFil);
