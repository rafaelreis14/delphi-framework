...
type
    ...
    public
        ...
        property Tipo_: string read GetTipo_;

        [TPersistable(fgPersistable, 'S10_STATUS')]
        [TMaxLenght(1)]
        property Status: string read FStatus write SetStatus;
        property Status_: string read GetStatus_;
...
uses Lib.Filter, Lib.Biblioteca, Biz.UsuarioLogado, Biz.Filial;
...
function TControleVersao.GetStatus_: string;
begin
    if Status = 'I' then
        result := 'INATIVA'
    else
...
function TControleVersao.GetTipo_: string;
begin
    if Tipo = 'C' then
        result := 'CAIXA'
    else
...
begin
    try
        ...
        campoFiltro.DataSource := TFilial.ObterTodasFiliais.ToList;
        ...
        campoFiltro.ListValue.Add('CAIXA=C');
        campoFiltro.ListValue.Add('CADASTRO=A');
        campoFiltro.ListValue.Add('LOJA=L');
        filtro.AddField(campoFiltro); 
        ...
        campoFiltro.GridSize := 100;
        campoFiltro.FieldType := fdListCombo;
        campoFiltro.ListValue.Add('ATIVO=A');
        campoFiltro.ListValue.Add('INATIVO=I');
        filtro.AddField(campoFiltro); 
