...
procedure TClasseBase.VerificarAlteracao;
var
    item: TPropriedadeAlterada;
begin
    for item in Self.PropriedadesAlteradas do
    begin
        if Self.VeririficarPropriedadeAlterada(item.NomePropriedade,Self.ObterValorPorNomePropriedade(item.NomePropriedade)) then
        begin
            FTipoManutencao := Alteracao;
            exit;
        end;
    end;
end;
...
begin
    ...
    try
        ...
        begin
            ...
            begin
                 if (_pro.PropertyType.ToString = 'string') or (_pro.PropertyType.ToString = 'Double') or
                    (_pro.PropertyType.ToString = 'Integer') or (_pro.PropertyType.ToString = 'Boolean') then
